...
## -- Creator Function for CALVING BEEF ----------------------------------------- ##

#' @title Comparison Plot Report Creator Function For Calving Beef (CALVING BEEF)
#'
#' @description
...
#' @examples
#' \dontrun{
#' create_ge_compare_plot_report_calving_beef(pn_cur_ge_label  = 2008,
#'                                  pn_prev_ge_label = 2004,
#'                                  pb_debug = TRUE)
#' }
#'
#' @export create_ge_compare_plot_report_calving_beef
create_ge_compare_plot_report_calving_beef <- function(pn_cur_ge_label,
                                                       pn_prev_ge_label,
  ...
  if (pb_debug) {
    if (is.null(plogger)){
      lgr <- get_qgert_logger(ps_logfile = 'create_ge_compare_plot_report_calving_beef.log', ps_level = 'INFO')
    } else {
      lgr <- plogger
    }
    qgert_log_info(plogger = lgr, ps_caller = 'create_ge_plot_report',
                   ps_msg = " Start of function create_ge_compare_plot_report_calving_beef ... ")
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_calving_beef",
  ...
  if (is.null(l_plot_opts)){
    l_plot_opts <- get_default_plot_opts_calving_beef()
  }
  ...
  s_rep_path <- file.path(s_ge_dir, paste0('ge_plot_report_calving_beef.Rmd', collapse = ''))
  if (pb_debug)
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_calving_beef",
  ...
  if (pb_debug)
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_calving_beef",
             ps_msg    = " * End of function create_ge_compare_plot_report_calving_beef")
