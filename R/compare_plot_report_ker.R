...
###
###
###   Purpose:   Create a Comparison Plot Report for KER
###   started:   2019-10-14 (pvr)
###
### ########################################################## ###


## -- Creator Function for KER ----------------------------------------- ##

#' @title Comparison Plot Report Creator Function For Aufzuchtverlust (KER)
#'
#' @description
...
#' @examples
#' \dontrun{
#' create_ge_compare_plot_report_ker(pn_cur_ge_label  = 1908,
#'                                   pn_prev_ge_label = 1904,
#'                                   pb_debug = TRUE)
#' }
#'
#' @export create_ge_compare_plot_report_ker
create_ge_compare_plot_report_ker <- function(pn_cur_ge_label,
                                              pn_prev_ge_label,
  ...
  if (pb_debug) {
    if (is.null(plogger)){
      lgr <- get_qgert_logger(ps_logfile = 'create_ge_compare_plot_report_ker.log', ps_level = 'INFO')
    } else {
      lgr <- plogger
    }
    qgert_log_info(plogger = lgr, ps_caller = 'create_ge_plot_report',
                   ps_msg = " Start of function create_ge_compare_plot_report_ker ... ")
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_ker",
  ...
  if (is.null(l_plot_opts)){
    l_plot_opts <- get_default_plot_opts_ker()
  }
  ...
  for (breed in vec_breed){
    ...
    for (sex in l_plot_opts$vec_sex){
      ...
      s_report_text  <- glue::glue('## Comparison Of Plots\n',
                                   'Plots compare estimates of resistence to ketosis (ker) for {tolower(sex)}',
                                   ' of breed {breed}',
      ...
      s_rep_path <- file.path(s_ge_dir, paste0('ge_plot_report_ker_compare', sex, '_', breed, '.Rmd', collapse = ''))
      if (pb_debug)
        qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_ker",
  ...
  if (pb_debug)
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_ker",
             ps_msg    = " * End of function create_ge_compare_plot_report_ker")
