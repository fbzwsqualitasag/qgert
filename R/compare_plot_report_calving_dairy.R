...
###   started:   2021-12-07 (skn)
###
### ########################################################## ###


## -- Creator Function for CALVING DAIRY ----------------------------------------- ##

#' @title Comparison Plot Report Creator Function For Calving Dairy (CALVING DAIRY)
#'
#' @description
...
#' @examples
#' \dontrun{
#' create_ge_compare_plot_report_calving_dairy(pn_cur_ge_label  = 2008,
#'                                  pn_prev_ge_label = 2004,
#'                                  pb_debug = TRUE)
#' }
#'
#' @export create_ge_compare_plot_report_calving_dairy
create_ge_compare_plot_report_calving_dairy <- function(pn_cur_ge_label,
                                                       pn_prev_ge_label,
  ...
  if (pb_debug) {
    if (is.null(plogger)){
      lgr <- get_qgert_logger(ps_logfile = 'create_ge_compare_plot_report_calving_dairy.log', ps_level = 'INFO')
    } else {
      lgr <- plogger
    }
    qgert_log_info(plogger = lgr, ps_caller = 'create_ge_plot_report',
                   ps_msg = " Start of function create_ge_compare_plot_report_calving_dairy ... ")
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_calving_dairy",
  ...
  if (is.null(l_plot_opts)){
    l_plot_opts <- get_default_plot_opts_calving_dairy()
  }
  ...
  s_rep_path <- file.path(s_ge_dir, paste0('ge_plot_report_calving_dairy.Rmd', collapse = ''))
  if (pb_debug)
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_calving_dairy",
  ...
  if (pb_debug)
    qgert_log_info(plogger = lgr, ps_caller = "create_ge_compare_plot_report_calving_dairy",
             ps_msg    = " * End of function create_ge_compare_plot_report_calving_dairy")
